...
loop_reverse(){
    ...
    if (m == NULL || m->mark == WALL || m->closed){
        return (void*)0;   /* Not valid, or closed already. */
    }
    if (m->opened && curR->Rgs + 1 >= m->Rgs){
        return (void *)0;    /* Old node met, not getting shorter. */
    }
...
forward_loop(){
    ...
    if (n == NULL || n->mark == WALL || n->closed){
        continue_flagF = 1;/* Not valid, or closed already. */
        return;
    }
    if (n->opened && cur->Fgs + 1 >= n->Fgs){
        continue_flagF = 1;/* Old node met, not getting shorter. */
        return;
    }
