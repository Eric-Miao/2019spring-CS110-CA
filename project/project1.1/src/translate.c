...
unsigned write_pass_one(FILE* output, const char* name, char** args, int num_args) {
  ...
  /*expand li*/
  if (strcmp(name, "li") == 0) {
    /*return if num_args not proper*/
    if(num_args!=2)return 0;
    long num;
    if(translate_num(&num,args[1],2047,-2048)==0){
      /*num field in one addi instruction*/
      sub_args[0]=args[0];
      sub_args[1]="x0";
      sub_args[2]=args[1];
      /*addi args[0], x0, args[1]*/
      write_inst_string(output,"addi",sub_args,3);
      return 1;

    }else{
      if(translate_num(&num,args[1],2147483647,-2147483648)!=0){
        /*check if num not in 2^31-1 field (error)*/
        return 0;
      } 
      /*number for lui */
      int num_lui=(((uint32_t)(num))>>12);
      sub_args[1]=malloc(100);

      /*number for addi*/
      int num_addi=(((uint32_t)(num))<<20)>>20;
      if( (num_addi&0x800) !=0){
        /*deal with the sign bit of addi*/
        num_lui++;
        num_addi=-2048+(num_addi&0x7FF);
      }

      sub_args[0]=args[0];
      sprintf(sub_args[1],"%d",num_lui);
      /*lui args[0], num_lui*/
      write_inst_string(output,"lui",sub_args,2);

      strcpy(sub_args[1],args[0]);
      sub_args[2]=malloc(100);
      sprintf(sub_args[2],"%d",num_addi);
      /*lui args[0], num_addi*/
      write_inst_string(output,"addi",sub_args,3);
      
      free(sub_args[1]);free(sub_args[2]);
      return 2;
    }
  ...
  /*return 0 with no writing error?*/
  return 0;
}
...
int translate_inst(FILE* output, const char* name, char** args, size_t num_args, uint32_t addr, SymbolTable* symtbl) {
        if(strcmp(name,"lb")==0)return write_itype(0x03,0x00,output,args,num_args);
    ...
    else if(strcmp(name,"ecall")==0)return write_itype(0x73,0x00,output,args,num_args);
    else if(strcmp(name,"ebreak")==0)return write_itype(0x73,0x00,output,args,num_args);
    ...
    else if(strcmp(name,"add")==0)return write_rtype(0x33,0x00,0x00,output,args,num_args);
...
int write_rtype(uint8_t opcode, uint8_t funct3, uint8_t funct7, FILE* output, char** args, size_t num_args) {
  ...
  instruction=(funct7<<25)+(rt<<20)+(rs<<15)+(funct3<<12)+(rd<<7)+opcode;
...
int write_itype(uint8_t opcode, uint8_t funct3, FILE* output, char** args, size_t num_args) {
  ...
  /*error args*/
  if(num_args!=3)return -1;
  if(args[0][0]>'z'||args[0][0]<'a')return -1;
  if((args[1][0]>='a'&&args[1][0]<='z')&&(args[2][0]>='a'&&args[2][0]<='z'))return -1;

  /*get imm from right agrs*/
  long int imm;
  if(args[1][0]>='a'&&args[1][0]<='z'){
    /*args[1] is a reg */
    rs = translate_reg(args[1]);
    if(translate_num(&imm,args[2],2047,-2048)==-1){
      return -1;
    }
  }else{
    /*args[2] is a reg */
    rs = translate_reg(args[2]);
    if(translate_num(&imm,args[1],2047,-2048)==-1){
      return -1;
    }
  }
  rd = translate_reg(args[0]);
  ...
  /*generate*/
  instruction=(((uint32_t)imm)<<20)+(rs<<15)+(funct3<<12)+(rd<<7)+opcode;
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_stype(uint8_t opcode, uint8_t funct3, FILE* output, char** args, size_t num_args) {
  ...
  /*get imm and check field*/
  long int imm;
  if(translate_num(&imm,args[1],2047,-2048)==-1){
    return -1;
  }
  
  instruction=((((uint32_t)imm)>>5)<<25)+(rt<<20)+(rs<<15)+(funct3<<12)+((((uint32_t)imm)<<27)>>20)+opcode;
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_sbtype(uint8_t opcode, uint8_t funct3, FILE* output, char** args, size_t num_args,
  ...
  /*args[2] label       addr current byte_offset*/
  label=get_addr_for_symbol(symtbl,args[2]);
  if(label==-1)return -1;

  /*get imm and seperate bit*/
  imm=label-addr;
  imm11=imm&(0x800);
  imm12=imm&(0x1000);
  /*generate*/
  instruction=(imm12<<19)+(((uint32_t)imm>>5)<<26>>1)+(rt<<20)+(rs<<15)+(funct3<<12)+((uint32_t)imm>>1<<28>>20)+(imm11>>4)+opcode;
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_utype(uint8_t opcode, FILE* output, char** args, size_t num_args) {
  ...
  /*get imm and check field*/
  long int imm;
  if(translate_num(&imm,args[1],1048575,0)==-1){
    return -1;
  }

  instruction=(((uint32_t)imm)<<12)+(rd<<7)+opcode;
  write_inst_hex(output, instruction);
  return 0;
}
...
int write_ujtype(uint8_t opcode, FILE* output, char** args, size_t num_args,
  ...
  if(rd==-1)return -1;
      ...
      /*args[1] label       addr current byte_offset*/
  label=get_addr_for_symbol(symtbl,args[1]);
  if(label==-1)return -1;

  /*get imm and imm bits*/
  imm=label-addr;
  imm20=imm&(0x100000);
  imm11=imm&(0x800);

  instruction=(imm20<<11)+((uint32_t)imm>>1<<22>>1)+(imm11<<9)+((uint32_t)imm>>12<<24>>12)+(rd<<7)+opcode;
  write_inst_hex(output, instruction);
  return 0;
}
