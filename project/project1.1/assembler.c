...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
  ...
  uint32_t line_num=0,byte_offset=0;
  int label_tag;
    /* 0 succ -1 error */
  int status=0;
  char *args[MAX_ARGS];
  ...
  while(fgets(buf,sizeof(buf),input)){
      ...
      /* deal with label token*/
      label_tag=add_if_label(line_num,token,byte_offset,symtbl);
      ...
      if(label_tag==-1){
        ...
        name=token;
          ...
          /*label and get instruction name*/
      ...
      while((token=strtok(NULL,IGNORE_CHARS))!=NULL){
        ...
        /*if there is error, dont write*/
      if(name!=NULL&&arg_cor!=-1){  
          int ac=write_pass_one(output,name,args,(int)num_args);
          if(ac==0){
              /*if write nothing, it must be a wrong pseudo-ins*/
              status=-1;
              raise_instruction_error(line_num,name,args,(int)num_args);
          }
          /*byte_offset=4*sentences */
          byte_offset+=4*ac;
      }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl) {
    ...
    uint32_t line_num=0,byte_offset=-4;
    ...
    /* get instruction name */
       ...
       the instruction will not be written and you should call
        ...
        if(translate_inst(output,name,args,num_args,byte_offset,symtbl)!=0){
            raise_instruction_error(line_num,name,args,(int)num_args);
            status=-1;
        }
    }
